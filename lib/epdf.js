...
const {delay, once, printToPDF, writeFile, errorUuid} = require('./utils')

const {url, out: filename, width, height, maxDelay, siteDelay, pageSize,
  printBackground, landscape, marginsType} = args
  ...
  .map(error => {
    if (error) {
      console.error(
        ...
        ? Error(`EPDF error ${errorUuid}: ${error.message}`)
        : `EPDF error ${errorUuid}: ${error}`
      )
    }
    ...
    // "guess" the error from stderr by looking for errorUuid.
    process.exit(0)
  })
}
